...
export default function ForgotPasswordPage() {
    ...
    const handleResetPassword = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setMessage("Access key reset link transmitted to your terminal.")
        } catch (err: any) {
            setError(err.message || "Protocol failure occurred")
        } finally {
            setLoading(false)
        }
    }
            ...
            <Card className="w-full max-w-[440px] bg-white/[0.03] backdrop-blur-[40px] border border-white/10 text-white shadow-[0_50px_100px_rgba(0,0,0,0.5)] relative z-10 overflow-hidden group hover:border-white/20 transition-all duration-700 rounded-[40px]">
                ...
                <CardHeader className="space-y-8 flex flex-col items-center pt-14 pb-4">
                    ...
                    <div className="text-center space-y-2">
                        <CardTitle className="text-4xl font-black tracking-tighter text-white drop-shadow-2xl uppercase">Recovery</CardTitle>
                ...
                <CardContent className="space-y-6 pt-6 px-10">
                    <form onSubmit={handleResetPassword} className="space-y-5">
                        ...
                        {message && (
                            <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-bold tracking-wider uppercase p-4 rounded-2xl text-center backdrop-blur-md animate-in slide-in-from-top-4 duration-500">
                                {message}
                            </div>
                        )}
                ...
                <CardFooter className="flex flex-col space-y-4 pt-6 pb-12 bg-white/[0.02] border-t border-white/5">
                    <Link to="/login" className="text-xs font-black tracking-widest uppercase text-white/30 hover:text-emerald-500 flex items-center justify-center gap-3 transition-all group/back">
